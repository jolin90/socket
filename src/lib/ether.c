...
void show_eth_header(struct ethhdr *ethhdr)
{
	...
	pr_std_info("dest_mac = %s", mac_address_tostring((const char *)ethhdr->h_dest, \
				sizeof(ethhdr->h_dest), (const char *)buff));
	pr_std_info("src_mac = %s", mac_address_tostring((const char *)ethhdr->h_source, \
				sizeof(ethhdr->h_source), (const char *)buff));
